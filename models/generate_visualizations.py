...
def plot_risk_distribution(df: pd.DataFrame) -> Path:
    ...
    sns.countplot(data=df, x="default_stage", order=df["default_stage"].value_counts().index)
...
def plot_association_rules(df: pd.DataFrame) -> Path:
    ...
    top_rules["rule_label"] = (
        top_rules["antecedents"].str.slice(0, 40) + " -> " + top_rules["consequents"].str.slice(0, 20)
    )
