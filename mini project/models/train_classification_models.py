"""
train_classification_models.py - Train and compare default prediction models.

This module trains three supervised classifiers and writes performance results.
"""
...
def load_data() -> pd.DataFrame:
    """Load preprocessed data and sample it if the dataset is too large."""
    ...
    if len(df) > MAX_SAMPLE_SIZE:
        default_distribution = df["default_flag"].value_counts(normalize=True)
        sampled_frames = []
        for default_value, weight in default_distribution.items():
            subset = df[df["default_flag"] == default_value]
            sample_size = max(1, int(round(MAX_SAMPLE_SIZE * weight)))
            sample_size = min(sample_size, len(subset))
            sampled_frames.append(subset.sample(n=sample_size, random_state=42))
...
def prepare_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    """Select the input features and target label for classification."""
    feature_columns = [
        "loan_amnt",
        "term",
        "int_rate",
    ...
    valid_features = [column for column in feature_columns if column in df.columns]
...
def train_and_evaluate(X: pd.DataFrame, y: pd.Series) -> dict:
    """Train three models and return their evaluation metrics."""
...
def write_report(results: dict) -> None:
    """Write classification results and metrics to markdown."""
    lines = [
...
def main() -> None:
    """Load data, train models, and save the evaluation results."""
    df = load_data()
