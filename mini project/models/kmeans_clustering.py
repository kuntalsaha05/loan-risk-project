"""
kmeans_clustering.py - Cluster borrower profiles into risk groups.

This module uses K-Means to group borrowers and then labels each group as
low, medium, or high risk based on default behavior and financial features.
"""
...
def load_data() -> pd.DataFrame:
    """Load a sample of cleaned loan data for clustering."""
...
def select_cluster_features(df: pd.DataFrame) -> pd.DataFrame:
    """Choose numeric features that help separate borrower risk groups."""
    selected_columns = [
        "loan_amnt",
        "int_rate",
        "annual_inc",
...
def build_pipeline() -> Pipeline:
    """Build the clustering pipeline with imputation, scaling, and K-Means."""
    return Pipeline(
        steps=[
...
def label_clusters_by_risk(df: pd.DataFrame, labels: pd.Series) -> tuple[pd.Series, dict]:
    """Rank clusters by risk and map cluster IDs to human-friendly labels."""
    summary = (
    ...
    labeled_series = labels.map(mapping)
    return labeled_series, mapping


def build_cluster_summary(df: pd.DataFrame, risk_labels: pd.Series) -> list[dict]:
    """Create a summary table of each labeled risk cluster."""
    cluster_df = df.copy()
...
def write_report(summary_records: list[dict]) -> None:
    """Write a human-readable markdown report for clustering results."""
    lines = [
...
def main() -> None:
    """Execute clustering and save the labeled results and summary."""
    df = load_data()
    ...
    risk_labels, label_mapping = label_clusters_by_risk(feature_df, cluster_ids)
    ...
    CLUSTER_SUMMARY_PATH.write_text(json.dumps(summary_payload, indent=2), encoding="utf-8")
