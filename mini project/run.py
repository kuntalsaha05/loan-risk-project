...
This script runs the full project end-to-end in the correct order.
It is meant to be used after installing dependencies in backend/requirements.txt.
"""
...
def run_step(label: str, command: list[str]) -> bool:
    """Run one pipeline step and print its status."""
    print(f"\n{'=' * 60}")
    print(f"  {label}")
    print(f"{'=' * 60}")
...
def main() -> None:
    """Execute all pipeline steps in the defined order."""
